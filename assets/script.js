...
function allStorage() {
    ...
    for (j = 0; j < values.length; j++) {
        $(".prev-list").prepend("<button class='button is-outlined' id='prev-city'>" + values[j] + "</button>");
    }
}
allStorage();
...
//This first ajax request collects current weather data and converts info into what we want to display.
    $.ajax({
        ...
        $(".prev-list").prepend("<button class='button is-outlined' id='prev-city'>" + subject + "</button>");
        ...
        for (i = 0; i < forecastTimes.length; i++) {
            if (forecastTimes[i].dt_txt[12] === "2") {
                ...
                if (forecastdisplay === false || forecastdisplay === undefined) {
                    $(".forecast-list").append("<div class='tile is-ancestor forecast-day'>" +
